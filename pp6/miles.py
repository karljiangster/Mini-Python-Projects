...
# Skeleton code for problem https://uchicago.kattis.com/problems/miles
#
...
#
<<<<<<< HEAD
def flights(itinerary, schedules, t_min, t_max, t = 0):
    
    if len(itinerary) <= 1: #should just be 1, though 
        return [["done"]] 
    
    start = itinerary[0]
    end = itinerary[1]

    routes = [] #list of lists of possible routes to take 

    times = schedules[start][end] 

    for depart, arrive in times: 
        if depart - t <= t_max and depart - t >= t_min: 
            next_flights = flights(itinerary[1:], schedules, t_min, t_max, arrive)             
            routes += list( ( map( lambda paths: [(depart, arrive)] + paths, next_flights) ) )
